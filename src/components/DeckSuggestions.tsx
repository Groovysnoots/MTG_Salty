    ...
    <div className="space-y-5 rounded-xl border border-zinc-800/80 bg-zinc-900/50 p-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1">
          ...
          {deck.commander && (
            <p className="text-sm text-zinc-400">
              Your commander: <span className="font-medium text-[#0B25FF]">{deck.commander.name}</span>
            </p>
          )}
        ...
        <button
          ...
          disabled={isLoading}
          className="rounded-xl bg-[#0B25FF] px-5 py-2.5 text-sm font-semibold text-white transition-all hover:bg-[#0B25FF]/80 disabled:cursor-not-allowed disabled:opacity-50 sm:self-start"
        >
          {isLoading ? (
        ...
        <div className="space-y-5">
          ...
          {suggestions.cardsToAdd.length > 0 && (
            <div className="space-y-2">
              <h4 className="flex items-center gap-2 text-xs font-semibold text-[#0B25FF] font-label">
                <span className="h-px flex-1 bg-[#0B25FF]/20" />
              ...
              {suggestions.cardsToAdd.map((s) => (
                <div
                  key={s.name}
                  className="rounded-xl border border-[#0B25FF]/20 bg-[#0B25FF]/5 p-3 transition-colors hover:border-[#0B25FF]/40"
                >
                  <div className="flex items-center gap-2">
                    <span className="text-[#0B25FF]">+</span>
          ...
          {suggestions.cardsToRemove.length > 0 && (
            <div className="space-y-2">
              <h4 className="flex items-center gap-2 text-xs font-semibold text-red-500 font-label">
