    ...
    <div className="space-y-5 rounded-xl border border-zinc-800/80 bg-zinc-900/50 p-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1">
          ...
          {deck.commander && (
            <p className="text-sm text-zinc-400">
              Your commander: <span className="font-medium text-amber-500">{deck.commander.name}</span>
            </p>
          )}
        ...
        <button
          ...
          disabled={isLoading}
          className="rounded-xl bg-amber-600 px-5 py-2.5 text-sm font-semibold text-white transition-all hover:bg-amber-500 disabled:cursor-not-allowed disabled:opacity-50 sm:self-start"
        >
          {isLoading ? (
